    ...
    if [ -f "$file" ]; then
        ...
        while IFS= read -r line; do
            if [[ "$line" == *".xt:"* ]]; then
                ...
                # Find corresponding panel for the test case's file
                FILE_PATH=$(echo "$TEST_CASE" | awk -F':' '{print $2}' | xargs)
                PANEL_ID="null"

                # Check if the file has a mapping in feedback-mapping.json
                ACTIVITY_ID=$(jq -r --arg file "$FILE_PATH" '.[] | select(.file == $file) | .activity' "$FEEDBACK_MAPPING_JSON")

                if [[ -n "$ACTIVITY_ID" && "$ACTIVITY_ID" != "null" ]]; then
                    # Find the panel that references this file within the correct activity
                    PANEL_ID=$(jq -r --arg activity "$ACTIVITY_ID" --arg file "$FILE_PATH" \
                        '.activities[] | select(.id == $activity) | .panels[] | select(.file == $file) | .id' "$ACTIVITY_JSON")
                fi

                # Debugging output (remove after confirming it works)
                echo "File: $FILE_PATH, Activity: $ACTIVITY_ID, Panel: $PANEL_ID"

                echo "      {" >> $OUTPUT_JSON
                echo "        \"test\": \"$TEST_CASE\"," >> $OUTPUT_JSON
                echo "        \"status\": \"failed\"," >> $OUTPUT_JSON
                echo "        \"error\": \"$ERROR_MESSAGE\"," >> $OUTPUT_JSON
                echo "        \"panel\": \"$PANEL_ID\"" >> $OUTPUT_JSON
                echo "      }" >> $OUTPUT_JSON
            fi
        done < <(grep -A3 "FAILURE!" "$file")
